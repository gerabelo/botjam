...
dtInicio = '08/07/2019'
dtFim = '05/08/2019'
pesquisaLivre = 'lucylea thome de paiva'
...
def getData(pagina):
    if pagina == 1:
        ...
        DATA = {
            ...
            'dadosConsulta.pesquisaLivre'   : pesquisaLivre,
            'pagina'                        : page
        }
        HEADERS = {
    ...
    for item in soup.findAll("tr", {"class": "ementaClass"}):
        for tds in item.findAll("td",recursive=False):
            for a in tds.findAll("a",recursive=False):
                data_urls.append(URLBASE+a.get('onclick').replace('return popup(\'','').replace('\')',''))
...
def init(data):
    ...
    while (total_de_paginas > 1):
        parsing(getData(total_de_paginas))
        total_de_paginas -= 1
